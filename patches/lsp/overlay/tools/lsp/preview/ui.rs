...
pub fn create_ui(
    ...
    api.on_copy_node_hierarchy(super::element_selection::copy_node_hierarchy);
    api.on_set_preview_language(|language| super::set_preview_language(language.to_string()));
    super::PREVIEW_STATE.with_borrow(|preview_state| {
        super::apply_preview_language(&preview_state.config);
        api.set_preview_language(preview_state.config.language.clone().into());
    });
