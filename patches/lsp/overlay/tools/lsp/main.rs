...
async fn handle_preview_to_lsp_message(
    ...
    match message {
        ...
        M::SetClipboard { text } => {
            ctx.server_notifier
                .send_notification::<common::SetClipboardNotification>(
                    common::SetClipboardParams { text },
                )?;
        }
        M::SetPreviewLanguage { language } => {
            ctx.preview_config.borrow_mut().language = language.clone();
            ctx.server_notifier
                .send_notification::<common::SetPreviewLanguageNotification>(
                    common::SetPreviewLanguageParams { language },
                )?;
        }
    }
    Ok(())
