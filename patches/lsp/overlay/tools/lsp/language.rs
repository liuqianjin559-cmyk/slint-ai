...
fn parse_configuration(workspace_config: Vec<serde_json::Value>) -> WorkspaceConfig {
    ...
    for config_value in workspace_config {
        if let Some(config_object) = config_value.as_object() {
            ...
            if let Some(lang) = config_object
                .get("preview")
                .and_then(|v| v.as_object()?.get("language")?.as_str())
            {
                language = lang.to_string();
            }
        }
    }
    WorkspaceConfig { hide_ui, include_paths, library_paths, style, experimental, language }
}
...
pub async fn load_configuration(ctx: &Context) -> common::Result<()> {
    ...
    let WorkspaceConfig { hide_ui, include_paths, library_paths, style, experimental, language } =
        workspace_config;

    let client_locale = ctx.init_param.locale.clone().unwrap_or_else(|| "en".into());
    let document_cache = &mut ctx.document_cache.borrow_mut();
    let mut diag = BuildDiagnostics::default();
