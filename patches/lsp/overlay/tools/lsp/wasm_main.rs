...
impl SlintServer {
    ...
    pub async fn process_preview_to_lsp_message(
        ...
        match message {
            ...
            M::SetClipboard { text } => {
                let _ = self.ctx.server_notifier.send_notification::<
                    crate::common::SetClipboardNotification,
                >(crate::common::SetClipboardParams { text });
            }
            M::SetPreviewLanguage { language } => {
                self.ctx.preview_config.borrow_mut().language = language.clone();
                let _ = self.ctx.server_notifier.send_notification::<
                    crate::common::SetPreviewLanguageNotification,
                >(crate::common::SetPreviewLanguageParams { language });
            }
        }
        Ok(())
    }
