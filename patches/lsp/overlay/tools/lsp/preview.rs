...
fn config_changed(config: PreviewConfig) {
    let Some((current, config, language_only)) = PREVIEW_STATE.with_borrow_mut(move |preview_state| {
        let previous = preview_state.config.clone();
        if previous == config {
            return None;
        }
        preview_state.config = config.clone();

        let language_only = previous.hide_ui == config.hide_ui
            && previous.style == config.style
            && previous.include_paths == config.include_paths
            && previous.library_paths == config.library_paths
            && previous.format_utf8 == config.format_utf8
            && previous.enable_experimental == config.enable_experimental
            && (previous.language != config.language
                || previous.client_locale != config.client_locale);
    ...
    PREVIEW_STATE.with_borrow(|preview_state| {
        if let Some(ui) = preview_state.ui.as_ref() {
            ui.global::<ui::Api>().set_preview_language(config.language.clone().into());
        }
    });
...
pub fn resolve_preview_language(configured: &str, client_locale: &str) -> String {
    match configured.trim().to_lowercase().as_str() {
        "auto" | "" => {
            if client_locale.to_lowercase().starts_with("zh") {
                "zh-cn".to_string()
            } else {
                "en".to_string()
            }
        }
        "zh-cn" | "zh" | "zh-tw" | "zh-hans" => "zh-cn".to_string(),
        lang => lang.to_string(),
    }
}

pub fn apply_preview_language(config: &PreviewConfig) {
    let effective = resolve_preview_language(&config.language, &config.client_locale);
    let _ = slint::select_bundled_translation(&effective);
}

pub fn set_preview_language(language: String) {
    PREVIEW_STATE.with_borrow_mut(|preview_state| {
        preview_state.config.language = language.clone();
        apply_preview_language(&preview_state.config);
        if let Some(ui) = preview_state.ui.as_ref() {
            ui.global::<ui::Api>().set_preview_language(language.clone().into());
        }
    });
    PREVIEW_STATE.with_borrow(|preview_state| {
        if let Some(to_lsp) = preview_state.to_lsp.borrow().as_ref() {
            let _ = to_lsp.send(&PreviewToLspMessage::SetPreviewLanguage { language });
        }
    });
}
